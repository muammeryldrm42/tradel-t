...
export function calcLiquidationPrice(entryPrice: string, leverage: number, side: "LONG" | "SHORT", maintenanceMarginRate: number): string {
  ...
  if (side === "LONG") return entry.mul(new Decimal(1).minus(new Decimal(1).div(lev)).plus(maintenanceMarginRate)).toFixed(8);
  return entry.mul(new Decimal(1).plus(new Decimal(1).div(lev)).minus(maintenanceMarginRate)).toFixed(8);
}

export function liquidationDistancePct(entryPrice: string, liquidationPrice: string, side: "LONG" | "SHORT"): number {
  const entry = new Decimal(entryPrice);
...
export function calcPnl(side: "LONG" | "SHORT", entryPrice: string, exitPrice: string, size: string): string {
  const entry = new Decimal(entryPrice);
...
export function calcSortino(returns: number[], riskFreeRate = 0): number {
  ...
  const downVar = negReturns.reduce((sum, r) => sum + Math.pow(r - riskFreeRate, 2), 0) / negReturns.length;
...
export function intervalToMs(interval: string): number {
  const map: Record<string, number> = { "1m": 60000, "5m": 300000, "15m": 900000, "30m": 1800000, "1h": 3600000, "4h": 14400000, "1d": 86400000 };
